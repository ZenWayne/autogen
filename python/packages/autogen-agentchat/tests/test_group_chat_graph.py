...
@pytest.mark.asyncio
async def test_digraph_group_chat_forward_loop_with_exit_condition(runtime: AgentRuntime | None) -> None:
    # Agents A and C: Echo Agents
    agent_a = _EchoAgent("A", description="Echo agent A")
    agent_c = _EchoAgent("C", description="Echo agent C")
    ...
    graph = DiGraph(
        nodes={
            #"A": DiGraphNode(name="A", edges=[DiGraphEdge(target="B")]),
            "B": DiGraphNode(
                name="B", edges=[DiGraphEdge(target="D", condition="exit"), DiGraphEdge(target="C", condition="loop")]
            ),
            "C": DiGraphNode(name="C", edges=[DiGraphEdge(target="B", condition="loop")]),
            "D": DiGraphNode(name="D", edges=[]),
        },
        default_start_node="B",
    )
...
@pytest.mark.asyncio
async def test_digraph_group_chat_loop_with_exit_condition_3(runtime: AgentRuntime | None) -> None:
    # Agents A and C: Echo Agents
    agent_a = _EchoAgent("A", description="Echo agent A")
    agent_c = _EchoAgent("C", description="Echo agent C")
